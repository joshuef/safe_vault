...
impl ChunkStorage {
    ...
    async fn try_store(&mut self, data: &Blob, origin: &MsgSender) -> Result<ResultMessage<()>> {
        ...
        if data.is_unpub() {
            ...
            if data_owner != id {
                ...
                return Ok(Err(ErrorMessage::InvalidOwners(id)));
            }
        }
        ...
        if self.chunks.has(data.address()) {
            ...
            return Ok(Err(ErrorMessage::DataExists));
        }
