...
use sn_messaging::{
    ...
    NodeQueryResponse, QueryResponse, Result as ResultMessage,
};
...
impl ChunkStorage {
    ...
    pub(crate) async fn store(
        ...
        if let Err(message_error) = self.try_store(data, origin).await {
            return self
                .wrapping
    ...
    pub(crate) async fn take_replica(
        ...
        let message = match self.try_store(data, origin).await {
            ...
            Err(message_error) => Message::NodeCmdError {
                id: MessageId::new(),
    ...
    async fn try_store(&mut self, data: &Blob, origin: &MsgSender) -> ResultMessage<()> {
        info!("TRYING TO STORE BLOB");
        let id = origin.id().public_key();

        if data.is_unpub() {
            let data_owner = *data.owner().ok_or(ErrorMessage::InvalidOwners(id))?;
            info!("Blob is unpub");
            info!("DATA OWNER: {:?}", data_owner);
            info!("ID OWNER: {:?}", id);
            if data_owner != id {
                info!("INVALID OWNER! Returning error");
                return Err(ErrorMessage::InvalidOwners(id));
            }
        }
        ...
        if self.chunks.has(data.address()) {
            ...
            return Err(ErrorMessage::DataExists);
        }

        // FIXME: change chunks.put to also return a ResultMessage<()>
        self.chunks
            .put(&data)
            .await
